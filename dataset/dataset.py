...
def get_datasets(
    batch_size, folder_path, train_folds=[1, 2, 3], val_folds=[4], test_folds=[5]
):
    metadata, _ = get_files(folder_path)
    train_s2_images, train_target_mask = get_slices(
        metadata, folder_path, folds=train_folds
    )
    val_s2_images, val_target_mask = get_slices(metadata, folder_path, folds=val_folds)
    test_s2_images, test_target_mask = get_slices(
        metadata, folder_path, folds=test_folds
    )

    prep = preprocess(folder_path, train_folds=train_folds)
    train_ds = (
        tf.data.Dataset.from_tensor_slices((train_s2_images, train_target_mask))
        .shuffle(1600)
        .map(prep, num_parallel_calls=tf.data.AUTOTUNE)
        .padded_batch(batch_size, padded_shapes=((61, 128, 128, 10), (128, 128)))
        .prefetch(tf.data.AUTOTUNE)
    )

    val_ds = (
        tf.data.Dataset.from_tensor_slices((val_s2_images, val_target_mask))
        .map(prep, num_parallel_calls=tf.data.AUTOTUNE)
        .padded_batch(batch_size, padded_shapes=((61, 128, 128, 10), (128, 128)))
        .prefetch(tf.data.AUTOTUNE)
    )
    ...
    test_ds = (
        tf.data.Dataset.from_tensor_slices((test_s2_images, test_target_mask))
        .map(prep, num_parallel_calls=tf.data.AUTOTUNE)
        .padded_batch(batch_size, padded_shapes=((61, 128, 128, 10), (128, 128)))
        .prefetch(tf.data.AUTOTUNE)
    )
